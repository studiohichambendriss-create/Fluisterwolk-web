...
default_calibration = {
    "whisper_threshold_value": 0.1,  # Adjust based on experimentation
...
def load_calibration():
    if os.path.exists(calibration_file):
        with open(calibration_file, 'r') as json_file:
            ...
            return {k: float(v) if isinstance(v, str) and v.replace('.', '', 1).isdigit() else v for k, v in calibration_data.items()}
...
def transcribe_audio(filename, language="nl"):
...
if __name__ == '__main__':
    while True:
        ...
        transcribe_audio(OUTPUT_FILENAME, language="nl")
