...
# Function to save the recording to a specified directory asynchronously
def save_recording(save_dir):
    ...
    # Use a background thread to save the file asynchronously
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(write_wave_file, file_path)
        # Optionally block until file is fully saved
        # future.result()  # Uncomment this line if you want to wait for the file to be saved

    print(f"Recording saved as {file_path}")
    return file_path

# Function to handle the actual wave file writing
...
def display_message(message, color=(255, 255, 255)):
    screen.fill((0, 0, 0))  # Clear the screen
    lines = message.split('\n')
    y_offset = 150  # Starting Y position
    for line in lines:
        text = font.render(line, True, color)
        screen.blit(text, (20, y_offset))
        y_offset += text.get_height() + 5  # Move to next line
    pygame.display.update()
...
def is_whisper_spectral_slope(audio_data, rate, slope_threshold=-0.005, plot=False):
    # Normalize audio data
    audio_data = audio_data / np.max(np.abs(audio_data))

    # Apply pre-emphasis filter to reduce noise and focus on speech range
    audio_data = librosa.effects.preemphasis(audio_data)

    # Calculate the STFT with improved resolution
    stft = np.abs(librosa.stft(audio_data, n_fft=2048, hop_length=256))

    # Get frequency bins
    freqs = librosa.fft_frequencies(sr=rate, n_fft=2048)

    # Mean amplitude for each frequency bin
    mean_amplitude = np.mean(stft, axis=1)

    # Avoid log of zero
    mean_amplitude[mean_amplitude == 0] = 1e-10

    # Convert to dB scale
    amplitude_db = 20 * np.log10(mean_amplitude)

    # Frequency limit (only consider frequencies up to 5000 Hz)
    freq_limit = freqs <= 5000
    freqs_limited = freqs[freq_limit]
    amplitude_db_limited = amplitude_db[freq_limit]

    # Linear regression to find the spectral slope
    X = freqs_limited.reshape(-1, 1)
    y = amplitude_db_limited
    X = np.hstack((X, np.ones_like(X)))
    coeffs = np.linalg.lstsq(X, y, rcond=None)[0]
    slope = coeffs[0]
    intercept = coeffs[1]

    print(f"Spectral slope: {slope}")

    # Calculate overall energy (RMS) of the audio signal
    rms_energy = np.sqrt(np.mean(audio_data**2))

    # Adjust threshold dynamically based on signal energy
    dynamic_threshold = slope_threshold + (rms_energy * 0.01)

    print(f"Dynamic threshold: {dynamic_threshold}")

    # Optionally plot the spectral slope
    if plot:
        plot_spectral_slope(freqs_limited, amplitude_db_limited, slope, intercept)

    # Determine if it's a whisper using the dynamic threshold
    is_whisper = slope > dynamic_threshold

    return is_whisper, slope

# Optional: Function to plot spectral slope (if you want to visualize the results)
def plot_spectral_slope(freqs, amplitude_db, slope, intercept):
    import matplotlib.pyplot as plt
    plt.figure()
    plt.plot(freqs, amplitude_db, label='Amplitude (dB)')
    plt.plot(freqs, slope * freqs + intercept, label=f'Slope: {slope:.5f}', color='red')
...
def handle_recording_confirmation(file_path):
    ...
    # Check if the audio is a whisper
    slope_threshold = -0.0065  # Adjust this value as needed
    is_whisper, spectral_slope_value = is_whisper_spectral_slope(
        audio_data, rate, slope_threshold=slope_threshold
    )
    ...
    # Display the spectral slope and threshold on the confirmation screen
    message = f"Spectral Slope: {spectral_slope_value:.6f}\nThreshold: {slope_threshold}\nIs Whisper: {'Yes' if is_whisper else 'No'}"
    display_message(message, (0, 255, 0))
...
def play_random_whispers():
    ...
    while True:
        if (pause_random_whispers_during_confirmation and is_in_confirmation) or is_recording:
        ...
        time.sleep(random.uniform(2.5, 3))  # Random interval between 1 and 3 seconds
