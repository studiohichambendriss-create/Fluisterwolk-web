...
def handle_recording_confirmation(file_path):
    ...
    audio_data = audio_data / np.max(np.abs(audio_data))  # Normalize audio data to -1 to 1 range

    # Crop silence from the audio
    cropped_audio = crop_silence(audio_data, rate, threshold=min_volume)
    normalized_audio = normalize_audio(cropped_audio, target_volume=max_volume)

    # Save the cropped and normalized audio back to the file
    wavfile.write(file_path, rate, (normalized_audio * 32767).astype(np.int16))  # Convert back to int16 format
    print("Audio cropped and normalized.")
...
# Function to play random whisper files from the "whispers" folder at random intervals
def play_random_whispers():
    while True:
        ...
        if whispers:
            selected_whisper = random.choice(whispers)
        ...
        time.sleep(random.uniform(1, 3))  # Random interval between 1 and 3 seconds
